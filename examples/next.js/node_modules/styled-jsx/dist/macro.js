...
function styledJsxMacro(_ref) {
  ...
  Object.keys(references).forEach(function (refName) {
    ...
    if (refName !== 'default' && refName !== 'resolve') {
      throw new _babelPluginMacros.MacroError("Imported an invalid named import: ".concat(refName, ". Please import: resolve"));
    ...
    references[refName].forEach(function (path) {
      ...
      if (templateExpression.isMemberExpression()) {
        ...
        if (refName !== 'default') {
          throw new _babelPluginMacros.MacroError("Can't use named import ".concat(path.node.name, " as a member expression: ").concat(path.node.name, ".").concat(tagPropertyName, "`div { color: red }` Please use it directly: ").concat(path.node.name, "`div { color: red }`"));
        } // Otherwise enforce `css.resolve`


        if (tagPropertyName !== 'resolve') {
          throw new _babelPluginMacros.MacroError("Using an invalid tag: ".concat(tagPropertyName, ". Please use ").concat(templateExpression.get('object').node.name, ".resolve"));
        ...
        if (refName === 'default') {
          ...
          throw new _babelPluginMacros.MacroError("Can't use default import directly eg. ".concat(name, "`div { color: red }`. Please use ").concat(name, ".resolve`div { color: red }` instead."));
        }
