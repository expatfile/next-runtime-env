  ...
  for (var i = attributes.length - 1, attr; attr = attributes[i]; i--) {
    ...
    if (t.isJSXSpreadAttribute(attr)) {
      if (t.isObjectExpression(node.argument)) {
        ...
        if (~index) {
          className = attr.get('argument').get("properties.".concat(index)); // Remove jsx spread attribute if there is only className property
      ...
      if (t.isMemberExpression(node.argument) || t.isIdentifier(node.argument)) {
        ...
        spreads.push( // `${name}.className != null && ${name}.className`
...
var validateExpressionVisitor = {
  MemberExpression: function MemberExpression(path, scope) {
    ...
    if (t.isIdentifier(node.property) && t.isThisExpression(node.object) && (node.property.name === 'props' || node.property.name === 'state' || node.property.name === 'context') || t.isIdentifier(node.object) && scope.hasOwnBinding(node.object.name)) {
      throw path.buildCodeFrameError("Expected a constant " + "as part of the template literal expression " + "(eg: <style jsx>{`p { color: ${myColor}`}</style>), " + "but got a MemberExpression: this.".concat(node.property.name));
    }
  },
  Identifier: function Identifier(path, scope) {
    ...
    var isDynamicBinding = false; // Traversing scope chain in order to find current variable.
...
var getJSXStyleInfo = function getJSXStyleInfo(expr, scope) {
  ...
  var css = quasis.reduce(function (css, quasi, index) {
    return "".concat(css).concat(quasi.value.cooked).concat(quasis.length === index + 1 ? '' : "%%styled-jsx-placeholder-".concat(index, "%%"));
  }, '');
  return {
    hash: hash,
    css: css,
...
var computeClassNames = function computeClassNames(styles, externalJsxId) {
  ...
  var staticClassName = "jsx-".concat(hashString(hashes.static.join(','))); // Static and optionally external classes. E.g.
  ...
  return {
    staticClassName: staticClassName,
    className: externalJsxId ? concat(concat(externalJsxId, t.stringLiteral(" ".concat(staticClassName, " "))), dynamic) : concat(t.stringLiteral("".concat(staticClassName, " ")), dynamic)
  };
};
...
var combinePlugins = function combinePlugins(plugins) {
  ...
  combinedPluginsCache.combined = plugins.map(function (plugin, i) {
    ...
    if (type !== 'function') {
      throw new Error("Expected plugin ".concat(plugins[i], " to be a function but instead got ").concat(type));
    }
    ...
    return function (css, babelOptions) {
      return plugin(previous ? previous(css, babelOptions) : css, _objectSpread({}, options, {
        babel: babelOptions
      }));
    };
  }, null);
...
var getPrefix = function getPrefix(isDynamic, id) {
  return isDynamic ? '.__jsx-style-dynamic-selector' : ".".concat(id);
};
...
var processCss = function processCss(stylesInfo, options) {
  ...
  var staticClassName = stylesInfo.staticClassName || "jsx-".concat(hashString(hash));
  var splitRules = options.splitRules;
  var useSourceMaps = Boolean(sourceMaps) && !splitRules;
  var pluginsOptions = {
    location: {
      start: _objectSpread({}, location.start),
      end: _objectSpread({}, location.end)
    },
...
var createReactComponentImportDeclaration = function createReactComponentImportDeclaration() {
  return t.importDeclaration([t.importDefaultSpecifier(t.identifier(_constants.STYLE_COMPONENT))], t.stringLiteral('styled-jsx/style'));
};
