  ...
  visitor: {
    JSXElement(path) {
      ...
      function getTitleElement(existingTitleChildren = []) {
        ...
        let expression = t.identifier('title'); // get the existing title string value

        const existingTitle = (existingTitleChildren || []).map(c => c.value).join();

        if (existingTitle) {
          // if title exists
          // render as follows
          // {title === undefined ? existingTitle : title}
          expression = t.conditionalExpression(t.binaryExpression('===', expression, t.identifier('undefined')), t.stringLiteral(existingTitle), expression);
        } // create a title element with the given expression


        return t.jsxElement(t.jsxOpeningElement(t.jsxIdentifier('title'), []), t.jsxClosingElement(t.jsxIdentifier('title')), [t.jsxExpressionContainer(expression)]);
